...
func createTestStateWithPolicy(t *testing.T) *State {
	...
	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "protect-main", []*tuf.Key{gpgKey}, []string{"git:refs/heads/main"}, 1)
	if err != nil {
		t.Fatal(err)
	}
	// Add a file protection rule. When used with common.AddNTestCommitsToSpecifiedRef, we have files with names 1, 2, 3,...n.
	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "protect-files-1-and-2", []*tuf.Key{gpgKey}, []string{"file:1", "file:2"}, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithDelegatedPolicies(t *testing.T) *State {
	...
	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "1", []*tuf.Key{key}, []string{"file:1/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}

	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "2", []*tuf.Key{key}, []string{"file:2/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	delegation1Metadata, err = AddOrUpdateDelegation(delegation1Metadata, "3", []*tuf.Key{gpgKey}, []string{"file:1/subpath1/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}

	delegation1Metadata, err = AddOrUpdateDelegation(delegation1Metadata, "4", []*tuf.Key{gpgKey}, []string{"file:1/subpath2/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithTagPolicy(t *testing.T) *State {
	...
	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "protect-tags", []*tuf.Key{gpgKey}, []string{"git:refs/tags/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
...
func createTestStateWithTagPolicyForUnauthorizedTest(t *testing.T) *State {
	...
	targetsMetadata, err = AddOrUpdateDelegation(targetsMetadata, "protect-tags", []*tuf.Key{rootKey}, []string{"git:refs/tags/*"}, 1)
	if err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
